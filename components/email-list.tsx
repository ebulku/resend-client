...
export function EmailList({ type, onEmailSelect }: EmailListProps) {
    ...
    async (forceRefresh = false) => {
      ...
      try {
        ...
        const response = await fetch(
          `/api/emails/${type}?apiKey=${encodeURIComponent(settings.apiKey)}`,
        );
  ...
  useEffect(() => {
    // Only update ref if type actually changed
    ...
    const timer = setTimeout(() => fetchEmails(false), 0);
          ...
          <Table>
            <TableHeader className="sticky top-0 bg-muted/50 backdrop-blur-sm z-10 border-b">
              <TableRow className="hover:bg-transparent">
                ...
                <TableHead className="w-[120px] font-semibold text-xs uppercase text-muted-foreground">
                  Date
                </TableHead>
                {type === "sent" && (
                  <TableHead className="w-[100px] font-semibold text-xs uppercase text-muted-foreground">
                    Status
                  </TableHead>
                )}
              </TableRow>
            ...
            <TableBody>
              {emails.map((email) => (
                <TableRow
                  ...
                  onClick={() => onEmailSelect?.({ ...email, folder: type })}
                >
            ...
            {emails.map((email) => (
              ...
              >
                ...
                {type === "sent" && email.status && (
                  <div className="mt-2">{getStatusBadge(email.status)}</div>
                )}
              </div>
            ))}
          </div>
