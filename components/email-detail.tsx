...
interface EmailDetailProps {
  email: Email | null;
  onBack: () => void;
  onReply?: (email: Email & { originalHtml?: string; originalText?: string; originalFrom?: string; originalDate?: string }) => void;
}
...
export function EmailDetail({ email, onBack, onReply }: EmailDetailProps) {
  ...
  const fetchEmailDetails = async (emailId: string, forceRefresh = false) => {
    ...
    try {
      ...
      let response = await fetch(
        `/api/emails/received/${emailId}?apiKey=${encodeURIComponent(settings.apiKey)}`
      );
  ...
  return (
    <div className="flex flex-col h-full bg-background">
      <div className="border-b bg-card/50 backdrop-blur-sm p-3 md:p-4 flex items-center gap-2 md:gap-4 sticky top-0 z-10 shadow-sm">
        <Button variant="ghost" size="icon" onClick={onBack} className="hover:bg-accent shrink-0 h-9 w-9 md:h-10 md:w-10">
          <ArrowLeft className="h-4 w-4" />
        </Button>
        <div className="flex-1 min-w-0">
          <h2 className="text-base md:text-xl font-semibold truncate mb-1">{displayEmail.subject || '(No subject)'}</h2>
          <div className="flex flex-col md:flex-row md:items-center gap-1 md:gap-2 text-xs md:text-sm text-muted-foreground">
            ...
            <span className="truncate">{Array.isArray(displayEmail.to) ? displayEmail.to.join(', ') : displayEmail.to}</span>
          </div>
        </div>
        <div className="flex items-center gap-1 md:gap-2 shrink-0">
          {loading && <RefreshCw className="h-4 w-4 animate-spin text-muted-foreground" />}
          {displayEmail.status && <div className="hidden md:block">{getStatusBadge(displayEmail.status)}</div>}
          {onReply && (
            <Button
              variant="default"
              size="sm"
      ...
      {error && (
        <div className="border-b p-3 bg-yellow-50 dark:bg-yellow-900/20">
        ...
        <div className="p-4 md:p-6 space-y-4 md:space-y-6 max-w-4xl mx-auto">
          <div className="space-y-2 md:space-y-3 bg-muted/30 rounded-lg p-3 md:p-4 border">
            <div className="flex flex-col md:flex-row md:items-start gap-1 md:gap-3 text-xs md:text-sm">
              <span className="font-semibold text-muted-foreground md:min-w-[60px]">From:</span>
              <span className="break-all">{displayEmail.from}</span>
            </div>
            <div className="flex flex-col md:flex-row md:items-start gap-1 md:gap-3 text-xs md:text-sm">
              <span className="font-semibold text-muted-foreground md:min-w-[60px]">To:</span>
              <span className="break-all">{Array.isArray(displayEmail.to) ? displayEmail.to.join(', ') : displayEmail.to}</span>
            </div>
            <div className="flex flex-col md:flex-row md:items-center gap-1 md:gap-3 text-xs md:text-sm">
              <span className="font-semibold text-muted-foreground md:min-w-[60px]">Date:</span>
            ...
            {displayEmail.status && (
              <div className="flex flex-col md:flex-row md:items-center gap-1 md:gap-3 text-xs md:text-sm md:hidden">
                <span className="font-semibold text-muted-foreground md:min-w-[60px]">Status:</span>
          ...
          {loading ? (
            ...
            <div className="prose dark:prose-invert max-w-none">
              {displayEmail.html ? (
                ...
                <pre className="whitespace-pre-wrap font-sans">{displayEmail.text}</pre>
              ) : (
